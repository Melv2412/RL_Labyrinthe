...
class QLearner:
    ...
    def choose_action(self, state: Tuple[int, int], grid: List[List[str]]) -> Optional[int]:
    ...
    def update_q_value(self, state: Tuple[int, int], action: int, 
                      reward: float, next_state: Tuple[int, int], grid: List[List[str]]) -> None:
    ...
    def get_best_action(self, state: Tuple[int, int], grid: List[List[str]]) -> Optional[int]:
    ...
    def train(self, grid: List[List[str]], start_pos: Tuple[int, int], 
        ...
        stagnation_limit = max(10, episodes // 10)
        best_total = None
        stagnation_count = 0
        ...
        for episode in range(episodes):
            ...
            # Stagnation check: arrêter si pas d'amélioration
            if best_total is None or total_reward > best_total:
